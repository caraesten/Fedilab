...
public class BackGroundTask extends AsyncTask<Void, Void, Void> {
    ...
    private void onRetrieveStreaming(Helper.EventStreaming event, final Account account, JSONObject response) {
        ...
        if( event == Helper.EventStreaming.NOTIFICATION){
            notification = API.parseNotificationResponse(contextReference.get(), response);
            ...
            final SharedPreferences sharedpreferences = contextReference.get().getSharedPreferences(Helper.APP_PREFS, Context.MODE_PRIVATE);
            boolean liveNotifications = sharedpreferences.getBoolean(Helper.SET_LIVE_NOTIFICATIONS, true);
            boolean canNotify = Helper.canNotify(contextReference.get());
            ...
            if((userId == null || !userId.equals(account.getId()) || activityPaused) && liveNotifications && canNotify && notify) {
                ...
                if( somethingToPush && notification != null){
                        ...
                        case "mention":
                            if(notif_mention){
                                if( notification.getAccount().getDisplay_name() != null && notification.getAccount().getDisplay_name().length() > 0 )
                                    title = String.format("%s %s", Helper.shortnameToUnicode(notification.getAccount().getDisplay_name(), true),contextReference.get().getString(R.string.notif_mention));
                                else
                                    title = String.format("@%s %s", notification.getAccount().getAcct(),contextReference.get().getString(R.string.notif_mention));
                            }
                            break;
                        case "reblog":
                            if(notif_share){
                                if( notification.getAccount().getDisplay_name() != null && notification.getAccount().getDisplay_name().length() > 0 )
                                    title = String.format("%s %s", Helper.shortnameToUnicode(notification.getAccount().getDisplay_name(), true),contextReference.get().getString(R.string.notif_reblog));
                                else
                                    title = String.format("@%s %s", notification.getAccount().getAcct(),contextReference.get().getString(R.string.notif_reblog));
                            }
                            break;
                        case "favourite":
                            if(notif_add){
                                if( notification.getAccount().getDisplay_name() != null && notification.getAccount().getDisplay_name().length() > 0 )
                                    title = String.format("%s %s", Helper.shortnameToUnicode(notification.getAccount().getDisplay_name(), true),contextReference.get().getString(R.string.notif_favourite));
                                else
                                    title = String.format("@%s %s", notification.getAccount().getAcct(),contextReference.get().getString(R.string.notif_favourite));
                            }
                            break;
                        case "follow":
                            if(notif_follow){
                                if( notification.getAccount().getDisplay_name() != null && notification.getAccount().getDisplay_name().length() > 0 )
                                    title = String.format("%s %s", Helper.shortnameToUnicode(notification.getAccount().getDisplay_name(), true),contextReference.get().getString(R.string.notif_follow));
                                else
                                    title = String.format("@%s %s", notification.getAccount().getAcct(),contextReference.get().getString(R.string.notif_follow));
                            }
                            break;
                        default:
                    }
                    //Some others notification
                    final Intent intent = new Intent(contextReference.get(), MainActivity.class);
                    ...
                    if( notification.getAccount().getAvatar() != null ) {
                        ...
                        File cacheDir = new File(contextReference.get().getCacheDir(), contextReference.get().getString(R.string.app_name));
                        ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(contextReference.get())
                                .imageDownloader(new PatchBaseImageDownloader(contextReference.get()))
                                .threadPoolSize(5)
                        ...
                        if( title != null) {
                            imageLoaderNoty.loadImage(notification.getAccount().getAvatar(), options, new SimpleImageLoadingListener() {
                                ...
                                public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
                                    ...
                                    notify_user(contextReference.get(), intent, notificationId, loadedImage, finalTitle, "@"+account.getAcct()+"@"+account.getInstance());
                                ...
                                public void onLoadingFailed(java.lang.String imageUri, android.view.View view, FailReason failReason) {
                                    notify_user(contextReference.get(), intent, notificationId, BitmapFactory.decodeResource(contextReference.get().getResources(),
            ...
            status = API.parseStatuses(contextReference.get(), response);
            status.setReplies(new ArrayList<fr.gouv.etalab.mastodon.client.Entities.Status>());
            status.setNew(true);
        ...
        LocalBroadcastManager.getInstance(contextReference.get()).sendBroadcast(intentBC);
    }
}
